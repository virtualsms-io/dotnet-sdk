...
/// <summary>
/// ISO-3166 alpha-2 to platform-tier numeric country ID map. Required only by
/// CreatePlatformRentalAsync (the backend's provider-tier create endpoint
/// takes a numeric ID; every other rentals endpoint resolves country_code
/// server-side). Not every ISO code the catalog lists is platform-tier
/// rental-capable -- an unmapped code means that country isn't available for
/// platform-tier rentals. Ported verbatim from the canonical client
/// (client.ts PLATFORM_TIER_COUNTRY_IDS) -- do not hand-edit without
/// re-syncing from there.
/// </summary>
public static class PlatformTierCountryIds
{
    public static readonly IReadOnlyDictionary<string, int> Map = new Dictionary<string, int>
    {
